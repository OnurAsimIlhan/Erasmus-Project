...
class ErasmusCoordinatorWaitingBin(MethodView, AuthorizeService):
    ...
    def get(self, department):
        if AuthorizeService.is_authorized(self):
            ...
            applications = self.applications_service.getApplicationsByStatus(status="waiting bin", dep=current_user.department)
            return render_template("erasmus_coordinator_waiting_bin.html", user = current_user, university_dictionary = university_dictionary, applications = applications)        
        else:
            logout_user() 
    ...
    def post(self, department):
        if AuthorizeService.is_authorized(self):
            ...
            if "place" in request.form:
                ...
                applications = self.applications_service.getApplicationsByStatus(status = "waiting bin", dep = current_user.department)
                return render_template("erasmus_coordinator_waiting_bin.html", application_period_id=app_period_id, user = current_user, university_dictionary=university_dictionary, applications = applications)
        else:
            logout_user() 
            return redirect(url_for("your_are_not_authorized_page"))